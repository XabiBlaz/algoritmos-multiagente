function [alpha,u] = alpha_strongWolfe_zoom(x,p,alphalo,alphahi,c1,c2,t,N,D,S,w0)
% ALPHA_STRONGWOLFE_ZOOM helps us to compute the alpha
% alphas = alpha_strongWolfe_zoom(func,grad,x,p,alphalo,alphahi,c1,c2)
% Algorithm 3.6 on page 61 in Nocedal and Wright
 fx = funcion(x);
 gx = gradient_point2D(x,t,N,D,S,w0)'*p;
 u=1;
 while 1
    alphax = 1/2*(alphalo + alphahi);
    xx = x + alphax*p;
    fxx = funcion(xx);
    gxx = gradient_point2D(xx,t,N,D,S,w0)'*p;
    xl = x + alphalo*p;
    fxl = funcion(xl);
    if ((fxx > funcion(xl)+c1*alphax*gx || (fxx >= fxl)))
      alphahi = alphax;
    else
      if abs(gxx) <= -c2*gx
        alpha = alphax;
        break;
      end
      if gxx*(alphahi-alphalo) >= 0
      alphahi = alphalo;
      end
      alphalo = alphax;
   end
   u=u+1;
   
end